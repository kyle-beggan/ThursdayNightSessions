...
export default function RecordingCommentsModal({ isOpen, onClose, recording, onCommentAdded }: RecordingCommentsModalProps) {
    ...
    }, [isOpen, recording, fetchComments]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!newMessage.trim() || !recording || isSubmitting) return;
    ...
    const handleDeleteComment = async (commentId: string) => {
        try {
            const res = await fetch(`/api/recordings/comments/${commentId}`, {
                method: 'DELETE'
            });
    ...
    return (
        <Modal
            isOpen={isOpen}
            onClose={onClose}
            title={recording ? `Conversation: ${recording.title}` : 'Conversation'}
            size="lg"
        >
