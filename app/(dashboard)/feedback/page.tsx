...
export default function FeedbackPage() {
    ...
    const openFeedback = feedbackList.filter(f => ['pending', 'in_progress'].includes(f.status));
    const closedFeedback = feedbackList.filter(f => ['completed', 'rejected'].includes(f.status));
                ...
                {isLoading ? (
                    ...
                    <div className="space-y-8">
                        {openFeedback.length > 0 && (
                            <div>
                                <div
                                    className="flex items-center justify-between mb-4 border-b border-border pb-2 cursor-pointer group"
                                ...
                                >
                                    ...
                                    <div
                                        className={`flex items-center justify-center w-8 h-8 rounded-full transition-all duration-300 ${isOpenExpanded
                                        ...
                                        style={{ transform: isOpenExpanded ? 'rotate(180deg)' : 'rotate(0deg)' }}
                                    >
                                ...
                                {isOpenExpanded && (
                                    <div className="space-y-4">
                                        {openFeedback.map(renderFeedbackItem)}
                                    </div>
                                )}
                            </div>
                        )}
                        ...
                        {closedFeedback.length > 0 && (
                            <div>
                                ...
                                >
                                    ...
                                    <div
                                        className={`flex items-center justify-center w-8 h-8 rounded-full transition-all duration-300 ${isClosedExpanded
                                            ? 'bg-surface-secondary text-text-secondary group-hover:bg-primary/20 group-hover:text-primary'
                                            : 'bg-primary text-white shadow-[0_0_10px_rgba(139,92,246,0.5)]'
                                            }`}
                                        style={{ transform: isClosedExpanded ? 'rotate(180deg)' : 'rotate(0deg)' }}
                                    >
                                        <FaChevronDown className="w-4 h-4" />
                                    </div>
                                </div>
                                {isClosedExpanded && (
                                    <div className="space-y-4">
                                        {closedFeedback.map(renderFeedbackItem)}
                                    </div>
                                )}
                            </div>
                        )}
